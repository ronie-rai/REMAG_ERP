...
import { generateDCMotorDataSheetPDF } from '../../utils/pdfGeneratorFallback'
import { format } from 'date-fns'
import { getComparator, stableSort } from '../../utils/tableSort'

function DCDataSheetList() {
  ...
  const fetchDataSheets = async () => {
    setLoading(true)
    try {
      const response = await productionAPI.getDCMotorDataSheets()
  ...
  const handleEdit = (id) => {
    navigate(`/production/dc-data-sheets/${id}/edit`)
  }

  const handleDelete = async (id) => {
    try {
      await productionAPI.deleteDCMotorDataSheet(id)
  ...
  const handlePDFExport = async (dataSheet) => {
    setPdfLoading(true)
    try {
      await generateDCMotorDataSheetPDF(dataSheet)
      setNotification({
        open: true,
  ...
  return (
    <Box sx={{ 
      ...
      <Paper sx={{ 
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            ...
            <Button 
              variant="contained" 
              onClick={() => navigate('/production/dc-data-sheets/new')}
              sx={{
                background: 'linear-gradient(45deg, #667eea, #764ba2)',
                '&:hover': {
              ...
              {sortedDataSheets.map((sheet, index) => (
                ...
                >
                  <TableCell>
                    <Button
                      variant="text"
                      onClick={() => navigate(`/production/dc-data-sheets/${sheet.id}`)}
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Are you sure you want to delete this DC motor data sheet? This action cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions>
...
export default DCDataSheetList
